...
var bdContacto = mysql.createConnection({
	host: "172.17.1.10",
	user: "sperez",
	password: "sp3r3z",
	database: "prollantas",
	...
	app.get('/reservacion', function(req, res){
		...
		guarda_conversa = "UPDATE conversaciones SET conversacion = '"+mensajes+"' WHERE clave = "+id;
			bdChat.query(guarda_conversa);*/
			
			/*Selecciona para mostrar en cada consola*/
		/*	conversacion_select ="SELECT Cliente, conversacion from conversaciones WHERE lConectado=1 and clave="+id;
				bdChat.query(conversacion_select, function(error, rows){
					if(error){
						console.log("Error: "+error.message);
						throw error;
					}else{
						//socket.broadcast.emit('mensajesxusuario', id, rows);
...
app.post('/conversacion', function(req, res){
	...
	disp_reservacion="SELECT * from disponibilidad WHERE disp=1";
	bdChat.query(disp_reservacion, function(error, rows){
		if(rows!=""){/*verifica que el usuario no exista, para no insertar dobles registros*/
		...
		bdChat.query(select_usu, function(error, rows1){
			if(rows1!=""){
				...
				/*Inserta en afcontactos(servidor 10) para publicidad de comercializacion*/
				insert_contact="INSERT INTO afcontactos(aNombreContacto, aCorreoElectronico, nClaveCorreo, lBaja,aOrigen,fFechaRegistro) values ('"+nombre+"','"+correo+"','0','0','CHAT','"+fechasinhora+"')";
				bdContacto.query(insert_contact);
	...
	insert_men="INSERT INTO conversaciones(asistente, empresa, Cliente, fecha, conversacion, emailCliente, asunto, puertoAsistente,lConectado) values ('nodisponible','Destinos Parhikuni','"+nombre+"','"+Fechaingreso+"','"+mensaje+"','"+correo+"','','1030','2')";
...
io.sockets.on('connection', function(socket){
	/**********Ingreso a reservaciones*********/
		socket.on('adduser',function(nombre_cl,tipo){
			...
			actualiza_conectado = "UPDATE conversaciones SET lconectado = 1 WHERE clave = "+nombre_cl;
		...
		socket.on('usu_desconectar',function(clave){
			...
			console.log('Este es el id:'+clave);
				actualiza_estado = "UPDATE conversaciones SET lconectado = 0 WHERE clave = '"+clave+"'";
		...
		socket.on('disconnect', function(){
			...
			if(socket.nombre_cl==1 || socket.nombre_cl==2){
				...
				desconecta_re = "UPDATE disponibilidad SET disp = 0 WHERE id = "+socket.nombre_cl;
				bdChat.query(desconecta_re);
			}
			io.sockets.emit('updaterooms', clientes_conect);
			select_desconectado = "SELECT clave, Cliente from conversaciones WHERE clave = '"+socket.nombre_cl+"'";
			bdChat.query(select_desconectado, function(error, rows1){
				if(error){
					...
					if(rows1!=""){
						socket.broadcast.emit('avisodesconecta',rows1[0].clave,rows1[0].Cliente);
					}
				}
			});
			
			
			actualiza_desconectado = "UPDATE conversaciones SET lconectado = 0 WHERE clave = '"+socket.nombre_cl+"'";
		...
		/*Guarda info si se refresca la pagina*/
		//socket.on('guardarinfo',function(id,mensajes){
		//console.log(' ides: '+id+' mensajes: '+mensajes);
		//	guarda_conversa = "UPDATE conversaciones SET conversacion = '"+mensajes+"' WHERE clave = "+id;
		//	bdChat.query(guarda_conversa);
		...
		/*Se da de baja el mensaje y selecciona los que hay para mostrarlos*/
		socket.on('bajamensaje',function(clave){
		
			oculta_mensaje = "UPDATE conversaciones SET lConectado = 3 WHERE clave = "+clave;
			bdChat.query(oculta_mensaje);
				obtenermensajes ="SELECT clave, Cliente, fecha, conversacion, emailCliente from conversaciones WHERE lconectado=2";
				bdChat.query(obtenermensajes,function(error,rows5){
				console.log(rows5[0]);
					socket.emit('agregarmensajes',rows5);
				});
		});
		
		/*obtener mensajes que dejaron los usuarios*/
		socket.on('mensajesmostrados',function(){
			obtenermensajes ="SELECT clave, Cliente, fecha, conversacion, emailCliente from conversaciones WHERE lconectado=2";
			bdChat.query(obtenermensajes,function(error,rows3){
				socket.emit('agregarmensajes',rows3);		
			});
		});	
});
